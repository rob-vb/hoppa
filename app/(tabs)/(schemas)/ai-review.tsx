...
export default function AIReviewScreen() {
  ...
  // Initialize state from extracted schema
  useEffect(() => {
    if (extractedSchema) {
      const result = convertExtractedSchemaToLocal(extractedSchema);
      if (result.success) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Clear the extracted schema from the store
      clearExtractedSchema();
      ...
      Alert.alert('Error', 'Failed to save schema. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDiscard = () => {
    Alert.alert(
      'Discard Changes',
      'Are you sure you want to discard this imported schema?',
      [
          ...
          {warnings.length > 0 && (
            <View style={styles.warningsContainer}>
              <View style={styles.warningsHeader}>
                <IconSymbol name="exclamationmark.triangle" size={18} color="#F59E0B" />
                <ThemedText style={styles.warningsTitle}>AI Notes</ThemedText>
              </View>
